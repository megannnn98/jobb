...
MAX_LENGTH = 192  # matches the len192 training run (kernel ruroberta-train-5class-len192)
BATCH_SIZE = 32
LIMIT = None  # full test set — GPU makes this cheap

# Temperature SWEEP: the main 128-token model's TEMPERATURE=0.25 is not portable between models —
# each retrain needs its own operating point. Logits are computed once;
# softmax sharpening is post-hoc, so grading the full threshold grid at every temperature is free.
...
def main() -> None:
    ...
    with (out_dir / "ruroberta_threshold_grid.txt").open("w", encoding="utf-8") as out:
        for temperature in TEMPERATURES:
            emit_note = (f"TEMPERATURE={temperature} applied (logits / TEMPERATURE before softmax) "
                         f"— see comment at top of file.\n")
            print(emit_note)
            out.write(emit_note + "\n")
