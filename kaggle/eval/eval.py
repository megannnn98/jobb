"""Held-out test evaluation of the ruRoberta-large sentiment model, on Kaggle GPU.

Same metrics logic as scripts/validate_sentiment_on_tsv.py (accuracy, macro F1,
per-class precision/recall/f1, confusion matrix, threshold grid) but:
  - auto-discovers the trained model dir from the ruroberta-train kernel's output
    (kernel_sources) and the test TSV from the rubert-job-reviews-sentiment dataset
  - runs batched inference on GPU
  - defaults to the FULL test set (no --limit) since GPU makes this fast, resolving
    the project's own docs/TODO.md item #8 ("run full test without --limit")
"""
...
def write_metrics_report(out, rows, probs, id2label, runtime_s) -> None:
    ...
    emit("=" * 80)
    emit("HELD-OUT TEST METRICS (argmax, ruRoberta-large, Kaggle GPU)")
    emit("=" * 80)
...
def write_threshold_grid(out, rows, probs, label2id) -> None:
    ...
    emit("=" * 80)
    emit("THRESHOLD GRID (production routing) — ruRoberta-large")
    emit("=" * 80)
...
def main() -> None:
    ...
    with (out_dir / "ruroberta_threshold_grid.txt").open("w", encoding="utf-8") as out:
        write_threshold_grid(out, rows, probs, label2id)
