#!/usr/bin/env python3
"""Build sentiment train/val/test TSV files from aj_reviews_export.jsonl."""
...
LABEL_MANUAL_REVIEW = "manual_review"


def map_label(row: dict) -> str:
    is_positive = row.get("isPositive") == 1
    is_published = row.get("status") == 1
...
def main() -> None:
    ...
    write_tsv(args.output_dir / "sentiment_train.tsv", train)
    write_tsv(args.output_dir / "sentiment_val.tsv", val)
    write_tsv(args.output_dir / "sentiment_test.tsv", test)
