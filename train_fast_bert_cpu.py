"""Fast two-phase RuBERT fine-tuning on CPU for sentiment classification.

Uses pre-split TSV datasets generated from aj_reviews_export.jsonl, the local
model/rubert-base-cased, and a three-class positive/negative/manual_review
label space. The distinguishing feature of this script is the two-phase
frozen-backbone schedule tuned for CPU training.
"""
...
MODEL_NAME = "model/rubert-base-cased"
TRAIN_FILE = Path("data/sentiment_train.tsv")
VAL_FILE = Path("data/sentiment_val.tsv")
OUTPUT_DIR = Path("outputs/rubert-sentiment-cpu")
...
TOKENIZE_NUM_PROC = int(os.environ.get("TOKENIZE_NUM_PROC", "1"))
...
PHASE2_BATCH_SIZE = 8
PHASE2_GRADIENT_ACCUMULATION = 4
PHASE2_EPOCHS = 1
PHASE2_LR = 2e-5


def configure_cpu_runtime() -> None:
    os.environ.setdefault("TOKENIZERS_PARALLELISM", "false" if TOKENIZE_NUM_PROC > 1 else "true")
...
def parse_tsv(path: Path, skip_short_text: bool) -> list[dict[str, str]]:
    ...
    with path.open(newline="", encoding="utf-8") as f:
        ...
        for line_number, row in enumerate(reader, start=2):
            ...
            if len(row) > len(EXPECTED_COLUMNS):
                # Best-effort recovery for unquoted tabs in text. This assumes id, name,
                # and label are tab-free; properly quoted tabs are handled by csv.reader.
...
def main() -> None:
    ...
    # -------------------------
    # ФАЗА 1: обучаем только classifier
    # -------------------------
    ...
    # -------------------------
    # ФАЗА 2: размораживаем последние 2 слоя
    # -------------------------
